...
$product_category_c = filter_input(INPUT_POST, 'c', FILTER_SANITIZE_NUMBER_INT);
$product_name_q = filter_input(INPUT_POST, 'q');
if (!isset($product_category_c))
    $product_category_c = 0;
...
if ($product_id == NULL) {
    ...
    $q = "";
    $c = "";
    if (isset($product_category_c) and $product_category_c != 0)
    {
        $c = "?c=".$product_category_c;
    }
    if (isset($product_name_q) and $product_name_q != "")
    {
        if (isset($product_category_c) and $product_category_c != 0)
            $q = "&";
        else
    ...
    header('Location: ../product_list/'.$c.$q);
    exit();
}
?>